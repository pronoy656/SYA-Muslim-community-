...
function LegalEditor() {
  ...
  const fetchDocument = async () => {
    try {
      const response = await axiosSecure.get(`/legal/${slug}`);
      ...
      toast.error("Failed to fetch legal document details");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      if (slug) {
        await axiosSecure.patch(`/legal/${slug}`, payload);
        toast.success("Legal document updated successfully!");
      } else {
        await axiosSecure.post("/legal", payload);
    ...
    <>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <h1 className="text-3xl font-normal text-slate-800 font-cinzel tracking-wider uppercase">
        ...
        <div className="flex items-center gap-3 font-sans">
          <Button 
            className="bg-[#C4A052] hover:bg-[#A8873A] text-white h-12 px-8 rounded-2xl flex items-center gap-2 shadow-sm transition-all font-semibold disabled:opacity-50"
